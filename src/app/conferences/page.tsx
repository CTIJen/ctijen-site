...
export default function ConferencesPage() {
    ...
    return (
      <>
            ...
            {cons.map((con, i) => (
              ...
              onMouseLeave={e => (e.currentTarget as HTMLElement).style.background = i % 2 === 0 ? 'var(--cream)' : 'var(--warm-white)'}>
                <div style={{ padding: '20px 24px 20px 48px', borderRight: '1px solid var(--border)' }}>
                  <a href={con.href} target="_blank" rel="noopener noreferrer" style={{
                ...
                <div style={{ padding: '20px 48px 20px 24px' }}>
                  ...
                  {con.jenNote && (
                    <p style={{
                      ...
                      fontSize: '13px', color: 'var(--dusty-pink-dim)', marginTop: '6px',
                    }}>☞ {con.jenNote}</p>
                  )}
                </div>
              </div>
        ...
        <ClosingNote
          text="Missing a con that should be here? Especially looking to add more regional and diversity-focused events."
          linkText="Suggest a con →"
          linkHref="/#contact"
        />
      </>
    );
