'use client';
import { PageHero, GradientRule, SectionHeader, JenNote, ClosingNote, Tag } from '@/components/resource-components';
...
function CapabilityTable({ title, note, rows }: {
  title: string;
  note?: string;
  ...
  return (
    <div style={{ marginBottom: '48px' }}>
      <h3 style={{
        ...
        marginBottom: note ? '6px' : '20px',
      }}>{title}</h3>
      {note && <p style={{
        fontSize: '13px', color: 'var(--mid)', fontWeight: 300,
          ...
          <thead>
            <tr style={{ borderBottom: '2px solid var(--border)' }}>
              <th style={{
                ...
                color: 'var(--mid)', padding: '10px 16px 10px 0',
                textAlign: 'left', fontWeight: 500, width: '220px',
              }}>Tool</th>
              {cols.map(col => (
                <th key={col} style={{
                  fontFamily: "'DM Mono', monospace", fontSize: '9px',
              ...
              <tr key={i} style={{
                ...
                <td style={{ padding: '14px 16px 14px 0', verticalAlign: 'top' }}>
                  <a href={row.href} target="_blank" rel="noopener noreferrer" style={{
                    fontWeight: 500, color: 'var(--ink)', textDecoration: 'none',
                  ...
                  <span style={{ fontSize: '12px', color: 'var(--mid)', fontWeight: 300, lineHeight: 1.5 }}>
                    {row.desc}
                  </span>
                </td>
                {keys.map(key => (
      ...
      <p style={{
        fontSize: '11px', color: 'var(--mid)', fontStyle: 'italic',
        marginTop: '8px', fontWeight: 300,
      }}>Capabilities may be limited on free versions</p>
    </div>
  );
}
...
export default function ToolsPage() {
  const tips = [
    { name: 'MISP', href: 'https://www.misp-project.org/', cost: 'Free + paid', effort: 'Easiest', desc: 'Open source threat data sharing platform. Perfect for beginners to dive into and see what a TIP is, how to correlate data, and practice collection and processing. Offers metadata tagging, feeds, visualization, and integrations.', jenNote: 'I may catch heat for saying it\'s easy to implement but I feel MISP is perfect for beginners. Tell me if you disagree!' },
    { name: 'OpenCTI', href: 'https://filigran.io/platforms/opencti/', cost: 'Free + paid', effort: 'Intermediate', desc: 'Great platform, but will take some technical know-how to install if you\'re going the free/self-hosted route. Very capable once running.' },
  ...
  return (
    <>
      <PageHero
        ...
        title="The Analyst"
        titleEm="Toolkit"
        desc="Every tool on this page is something Jen has used, evaluated, or recommends. No sponsorships, no affiliate links — just the real ones."
      />
      <GradientRule />
      ...
      <section style={{ padding: '80px 48px', background: 'var(--cream)' }}>
        <SectionHeader
          number="01 — Threat Intelligence Platforms"
          title="TIPs"
          desc="Ordered by ease of implementation — Jen's opinion."
        />
          ...
          {tips.map((tip, i) => (
            <div key={i} style={{
              ...
              <div style={{ padding: '20px 24px', borderRight: '1px solid var(--border)' }}>
                <a href={tip.href} target="_blank" rel="noopener noreferrer" style={{
                  fontFamily: "'Cormorant Garamond', serif",
              ...
              <div style={{
                ...
                <span style={{
                  ...
                  color: 'var(--mid)', letterSpacing: '0.04em',
                }}>{tip.effort}</span>
              </div>
              <div style={{ padding: '20px 24px' }}>
                <p style={{ fontSize: '13px', color: 'var(--mid)', lineHeight: 1.7, fontWeight: 300 }}>
                  {tip.desc}
                </p>
                {tip.jenNote && (
                  <div style={{
                    ...
                    borderLeft: '2px solid var(--gold)', borderRadius: '0 3px 3px 0',
                  }}>
                    <p style={{
                      fontFamily: "'Cormorant Garamond', serif", fontStyle: 'italic',
                      fontSize: '13px', color: 'var(--plum)', lineHeight: 1.6,
                    }}>{tip.jenNote}</p>
                  </div>
                )}
              </div>
      ...
      <section style={{ padding: '80px 48px', background: 'var(--warm-white)' }}>
        <SectionHeader
          number="02 — IOC & Analysis Tools"
          title="Capability Reference"
          desc="A quick-reference matrix for what each tool can handle. Great for bookmarking."
        />

        <CapabilityTable
          title="IOC Query & Enrichment"
          rows={[
        ...
        <CapabilityTable
          title="Malware Analysis & Sandboxes"
          rows={[
        ...
        <CapabilityTable
          title="More Tools Jen Uses Sometimes"
          rows={[
      ...
      <ClosingNote
        text="Missing a tool that should be here? Jen's always looking to expand this list."
        linkText="Suggest a tool →"
        linkHref="/#contact"
      />
    </>
  );
