...
export default function PodcastsPage() {
  const ctiPodcasts = [
    { name: 'Future of Threat Intelligence', href: 'https://podcasts.apple.com/us/podcast/future-of-threat-intelligence/id1631947902', desc: "Team Cymru explores the shift from reactive detection to proactive threat management. Top cybersecurity leaders sharing real strategies." },
    { name: 'Microsoft Threat Intelligence Podcast', href: 'https://podcasts.apple.com/us/podcast/microsoft-threat-intelligence-podcast/id1710656242', desc: 'Stories from the Microsoft Threat Intelligence community navigating APTs, cybercrime gangs, malware, vulnerabilities, and weird and cool tools and tactics.' },
  ];

  const generalPodcasts = [
    { name: 'Stats On Stats', href: 'https://www.youtube.com/@StatsOnStats', desc: "J and Tiffiny unpack complex cybersecurity issues and IT trends in an accessible way whether you're a professional or enthusiast." },
  ...
  return (
    <>
      ...
      <section style={{ padding: '80px 48px', background: 'var(--cream)' }}>
        <PageContainer>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: '12px', marginBottom: '64px' }}>
            {ctiPodcasts.map((p, i) => (
              <ResourceCard key={i}>
                <a href={p.href} target="_blank" rel="noopener noreferrer" style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: '18px', fontWeight: 700, color: 'var(--ink)', textDecoration: 'none', display: 'block', marginBottom: '10px' }}>{p.name} ↗</a>
      ...
      <ClosingNote text="Know a podcast that should be on this list? Send it over." linkText="Suggest one →" linkHref="/#contact" />
    </>
  );
}